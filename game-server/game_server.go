...
func NewGameServerWrapper() *GSWrapper {
	return &GSWrapper{}
}

func (gsw *GSWrapper) Ping(ctx context.Context, payload *PingPayload) (*PingResponse, error) {
...
func OpenNewConn(geAddr string) (*grpc.ClientConn, gameengine.RouteClient) {
	...
	if err != nil {
		fmt.Printf("it wasn't possible creating a client connection with the game engine in %s\n", geAddr)
	}
